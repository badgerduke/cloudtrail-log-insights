resource "null_resource" "install_dependencies" {
  provisioner "local-exec" {
    command = "pip install -r ${var.dist_root}/requirements.txt -t ${var.lambda_root}/"
  
  }
  
  # Will trigger this resource when dependencies or Lambda code changes
  triggers = {
    dependencies_versions = filemd5("${var.dist_root}/requirements.txt")
...
data "archive_file" "lambda_source" {
  ...
  output_path = "${var.dist_root}/${random_uuid.lambda_src_hash.result}.zip"
  type        = "zip"
}
